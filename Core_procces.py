...
class LoadReadyXlsx(QThread):
    ...
    progress_update = pyqtSignal(int)
    file_loaded = pyqtSignal(object)  # Сигнал для передачи загруженного workbook
    ...
    def run(self):
        self.progress_update.emit(63)  # Начальный прогресс
        workbook = openpyxl.load_workbook(self.file_path)
        path = self.file_path
        self.progress_update.emit(100)  # Завершаем прогресс
